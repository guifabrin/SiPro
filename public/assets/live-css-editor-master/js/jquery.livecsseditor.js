...
(function($)
{
    ...
    function assignEditor(props, propSelector, prop, valueContainer, editor, selectorIndex, propertyIndex){
        props[propSelector].editors[prop] = editor({
            ...
            ,'removePaddingColor':function(location){
                var element = preview.contents().find(propSelector);
                element.find(".padding"+selectorIndex + '-' + propertyIndex).remove();
                //preview.contents().find(propSelector).css('background-color',value);
            },
            'preview':preview,
            ...
            loadTpl(opts.props_tpl,function renderProperties(tplStr){
                ...
                for(var propSelector in props){
                    ...
                    for(var i = 0; i < props[propSelector].props.length; i++){
                        ...
                        //var query = '#properties-' + selectorIndex + ' .prop-index-' + i + ' > .lcePropValue';
                        var valueContainer = properties.find("#props"+selectorIndex+"p"+i+" > .lcePropValue").first();
    ...
    {
        'layout_tpl':'/assets/live-css-editor-master/tmpl/layout.html',
        'props_tpl':'/assets/live-css-editor-master/tmpl/properties.html'
    };
