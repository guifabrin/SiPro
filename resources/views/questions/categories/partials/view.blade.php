	...
	<li>
        ...
        @else
            <button class="btn btn-sm {{ isset($selected) && $selected->id == $categorie->id ? 'btn-primary' : 'btn-secondary' }} dropdown-toggle" id="siproDrodownMenu{{$categorie->id}}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                ...
                {{ $categorie->description }}
                <span class="badge badge-light">{{ $categorie->questions()->count() }}</span>
            </button>
            <div class="dropdown-menu" aria-labelledby="siproDrodownMenu{{$categorie->id}}">
                <a class="dropdown-item" href="{{ url('/questions/categorie/'.$categorie->id) }}">
                    <i class='fa fa-eye'></i> {{ _v('see') }}
                </a>
                <a class="dropdown-item" href="{{ url('/questions/categorie/'.$categorie->id.'/create') }}">
                    <i class='fa fa-plus'></i> {{ _v('add') }}
                </a>
                @if ($manage)
                    <a class="dropdown-item" href="{{ url('/questions/categories/confirm/'.$categorie->id) }}">
                        <i class='fa fa-times'></i> {{ _v('remove') }}
                    </a>
                    <a class="dropdown-item" href="{{ url('/questions/categories/'.$categorie->id) }}">
        ...
        @if(count($categorie->childrens) != 0)
            <ul>
                @include('questions.categories.partials.view', ['selected' => $selected, 'select' => $select, 'categories' => $categorie->childrens,
                 'manage' => $manage])
            </ul>
        @endif
	</li>
